...
def fetch_emp_education_for_pendidikan():
    query = """
            ...
            FROM emp_education AS eed
                     INNER JOIN emp_profile AS ep ON eed.emp_profile_id = ep.emp_profile_id
                     INNER JOIN sys_reference AS red ON eed.edu_level = red.`value` AND red.`code` = %s
...
def fetch_data_for_pendidikan():
    query = """
            ...
            """
    params = ("pendidikan", EmpWorkStatus.KaryawanAktif.value,)
    return fetch_smartoffice(query, params)
