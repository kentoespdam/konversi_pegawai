...
def fetch_employee_for_pegawai() -> pd.DataFrame:
    query = """
            SELECT em.emp_id                          AS pegawai_id,
                   ...
                   em.tmt_pensiun                     AS tmt_pensiun,
                   em.emp_sg_id                       AS gaji_profil_id,
                   em.emp_gp                          AS gaji_pokok,
                   IFNULL(em.emp_phdp, 0)             AS phdp,
                   ep.askes_flag                      AS is_askes,
                   ep.emp_tax_code                    AS emp_tax_code,
                   IFNULL(ep.id_rumdin, 0)            AS rumah_dinas_id,
                   em.jml_tanggungan                  AS jml_tanggungan,
                   em.mkg_tahun                       AS mkg_tahun,
                   em.mkg_bulan                       AS mkg_bulan,
...
def fetch_data_for_pegawai() -> pd.DataFrame:
    ...
    query = """
            SELECT emp.emp_id,
                   ...
                   CASE
                       ...
                       WHEN emp.emp_work_status = 8 THEN 'BERHENTI'
                       END                                                       AS statusKerja,
                   pos.pos_name                                                  AS namaJabatan,
                   org.org_name                                                  AS namaOrganisasi,
                   ''                                                            AS profesi,
...
def fetch_gaji_employee():
    query = """
            SELECT emp.emp_sg_id           AS gajiProfilId,
                   IFNULL(emp.emp_phdp, 0) AS phdp,
                   ep.id_rumdin            AS rumahDinasId,
                   emp.emp_code            AS nipam
            FROM employee AS emp
