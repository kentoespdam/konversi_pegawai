...
def fetch_emp_work_history_for_emp_sk():
    query = """
            ...
            FROM (SELECT em.emp_id,
                  ...
                  FROM emp_work_history AS ewh
                           LEFT JOIN emp_sk AS esk ON ewh.ewh_sk_no = esk.no_sk
                           INNER JOIN employee AS em ON ewh.emp_code = em.emp_code) AS ewh1
            WHERE ISNULL(esk_no_sk) \
            """
...
def fetch_data_for_riwayat_mutasi():
    query = """
            ...
            WHERE em.emp_flag IN (1, 2)
              AND (ep.emp_identity_number IS NOT NULL OR ep.emp_identity_number != '')
            ORDER BY ewh.ewh_id \
            """
    return fetch_smartoffice(query)
